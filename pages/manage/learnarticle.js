import {
    ...
    AlertDialogOverlay, Box, Button, Center, Flex, Heading, HStack, Input, InputGroup, InputRightElement, Table,
    TableContainer, Tbody, Td, Th, Thead, Tr, useDisclosure, useToast
...
export default () => {
    ...
    const onClickDelete = async () => {
        ...
        console.log(laid)
        let result = await axios.delete(`${url}/api/Manage/deleteManage/deleteLearnArti`, {
            headers: {
                learningarticleid: laid
            }
        })
        console.log(result)
        if (result) {
            toast({
                ...
                description: "Delete Article is complete",
                status: 'success',
                duration: 3000,
                isClosable: false,
            })
        ...
        <div className="">
            ...
            <Layout>
                <div className="p-4 mt-1">
                    <Heading size='2xl'>[Admin] Manage Learning Article</Heading>
                    <Box sx={line}></Box>
                                    ...
                                    <Table variant='simple'>
                                        ...
                                        <Tbody>{
                                            data.map((item, index) => {
                                                return (
                                                    <Tr key={index}>
                                                        <Td>{item.learningarticleID}</Td>
                                                        <Td>{item.topic}</Td>
                                                        <Td>{item.forskillID}</Td>
                                                        <Td>{item.dateadd}</Td>
                                                        <Td>
